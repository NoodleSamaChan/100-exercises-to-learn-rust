...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        ...
        if title.clone().into_bytes().len() > 50 {
            panic!("Title cannot be longer than 50 bytes")
        }
        if description.clone().into_bytes().len() > 500 {
            panic!("Description cannot be longer than 500 bytes")
        }
        if status != String::from("To-Do") && status != String::from("Done") && status != String::from("In Progress") {
